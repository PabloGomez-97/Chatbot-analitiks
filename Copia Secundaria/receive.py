...
def user_exists(user_number):
    conn = sqlite3.connect('user_responses.db')
    cursor = conn.cursor()
    cursor.execute("SELECT name, company FROM users WHERE user_number = ?", (user_number,))
...
def save_user(user_number, name, company):
    conn = sqlite3.connect('user_responses.db')
    cursor = conn.cursor()
    cursor.execute("INSERT INTO users (user_number, name, company) VALUES (?, ?, ?)", (user_number, name, company))
    conn.commit()
    conn.close()
